...
export default function WeatherDetailsMore({ weather }: ComponentProps) {
  return (
    <div className="bg-white border border-neutral-200 rounded-md shadow-md p-3 md:p-4">
      ...
      <WeatherDetailsMoreItem label="Humidity" value={`${weather.main.humidity}%`} />
      <WeatherDetailsMoreItem label="Wind speed" value={`${weather.wind.speed} km/h`} />
      <WeatherDetailsMoreItem label="Pressure" value={`${weather.main.pressure.toLocaleString("en-US")} mBar`} />
      {weather.rain && <WeatherDetailsMoreItem label="Rain volume" value={`${weather.rain["1h"]} mm/h`} />}
    </div>
  );
}
